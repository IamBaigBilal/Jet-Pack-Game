...
public class GUIManagerJS extends MonoBehaviour
{
    ...
    public function UseExtraSpeed(anim : Animator)
    {
        //If a powerup can't be activated, return
        if (!powerupManager.CanUsePowerup() || SaveManagerJS.extraSpeed <= 0)
            return;
    ...
    public function UseShield(anim : Animator)
    {
        //If a powerup can't be activated, return
        if (!powerupManager.CanUsePowerup() || SaveManagerJS.shield <= 0)
            return;
    ...
    public function UseSonicWave(anim : Animator)
    {
        //If a powerup can't be activated, return
        if (!powerupManager.CanUsePowerup() || SaveManagerJS.sonicWave <= 0)
            return;
